...
parser.add_argument('--out_dir', type=str, default='./TRADES_NoiLIn_', help='dir of output')
...
parser.add_argument('--LAMBDA', type=float,default=6)
...
def train(model, train_loader, optimizer, epoch, nr):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        x_adv = attack.pgd(model, data, noisy_labels, epsilon=8 / 255, step_size=2 / 255, num_steps=10,
                           loss_fn='kl',
                           category='trades', rand_init=True)
        ...
        loss = nn.CrossEntropyLoss(reduction='mean')(nat_output, noisy_labels) + args.LAMBDA * criterion_kl(
            F.log_softmax(output, dim=1), F.softmax(nat_output, dim=1)) / len(target)
        loss_sum += loss.item()
        loss.backward()
        optimizer.step()
...
if args.net == "ResNet18":
    model = ResNet18(num_class=num_classes).cuda()
    net = args.net
if args.net == "WRN":
...
if args.resume:
    ...
    print('==> TRADES-NoiLIn Resuming from checkpoint ..')
    print(args.resume)
